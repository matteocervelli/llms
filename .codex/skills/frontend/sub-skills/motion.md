...
Always specify exact properties. `transition: all` forces the browser to watch every property for
changes, causes unexpected transitions on properties you never intended to animate, and blocks
...
Note: `transition: transform` in CSS covers only the `transform` shorthand, not `scale`, `rotate`,
`translate` individually. List them explicitly if you're using the individual transform properties.

### `will-change` — GPU Hint, Use Sparingly

`will-change` pre-promotes an element to a GPU compositing layer before animation starts, preventing
first-frame stutter. Only useful for properties the GPU can composite:
...
Only add when you actually observe first-frame stutter. Each compositing layer costs GPU memory.
Modern browsers are already good at promoting animated elements on their own.
...
- [ ] `transition: all` used — specify exact properties instead
- [ ] `will-change: all` or `will-change` on non-GPU-compositable props (background, color)
